...
// Note that the length byte itself is not included included in the packet
// length
...
#define BASIC_RF_LEN_AUTH                                     \
    BASIC_RF_PACKET_OVERHEAD_SIZE + BASIC_RF_AUX_HDR_LENGTH - \
        BASIC_RF_FOOTER_SIZE
...
static uint8
    txMpdu[BASIC_RF_MAX_PAYLOAD_SIZE + BASIC_RF_PACKET_OVERHEAD_SIZE + 1];
 ...
 */
static uint8 basicRfBuildHeader(uint8* buffer,
                                uint16 destAddr,
                                uint8 payloadLength) {
    ...
    // pHdr->frameControlField = pConfig->ackRequest ? BASIC_RF_FCF_ACK :
    // BASIC_RF_FCF_NOACK;
 ...
 */
static uint8 basicRfBuildMpdu(uint16 destAddr,
                              uint8* pPayload,
                              uint8 payloadLength) {
    uint8 hdrLength, n;
...
static void basicRfRxFrmDoneIsr(void) {
    ...
    if (pHdr->packetLength == BASIC_RF_ACK_PACKET_SIZE) {
        ...
        authStatus =
            halRfReadRxBufSecure(&rxMpdu[1], pHdr->packetLength, rxi.length,
        ...
        // Notify the application about the received data packet if the CRC is
        // OK Throw packet if the previous packet had the same sequence number
        if ((pStatusWord[1] & BASIC_RF_CRC_OK_BM) &&
 ...
 * @fn          basicRfInit
 *
 * @brief       Initialise basic RF datastructures. Sets channel, short address
...
uint8 basicRfSendPacket(uint16 destAddr, uint8* pPayload, uint8 length) {
    ...
    if (pConfig->ackRequest) {
        ...
        // 12-symbol turnaround time, the ack packet duration, and a small
        // margin
        halMcuWaitUs((12 * BASIC_RF_SYMBOL_DURATION) + (BASIC_RF_ACK_DURATION) +
                     (2 * BASIC_RF_SYMBOL_DURATION) + 10);

        // If an acknowledgment has been received (by RxFrmDoneIsr), the
        // ackReceived flag should be set
